...
def slugify(name: str) -> str:
    ...
    之前淨係留 a-z0-9，於是「陳記茶餐廳」「小紅點設計」「日日鮮」全部變咗 `client`。
    對一個廣東話客戶群嚟講，咁即係第二個客會靜靜雞寫入第一個客個資料夾 ——
    唔會報錯，唔會有人發現，直到兩個客嘅紅線撈埋一齊。

    中日韓字元喺所有現代檔案系統同 git 都用得，所以留低佢：資料夾名讀得明，
    而且唔會撞。
    """
    s = (name or "").strip().lower()
    # 保留：英數、中日韓統一表意文字、擴充區A、兼容表意文字
    s = re.sub(r"[^a-z0-9㐀-䶿一-鿿豈-﫿]+", "-", s).strip("-")
    return s or "client"
